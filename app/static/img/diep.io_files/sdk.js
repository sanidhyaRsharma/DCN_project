m28n=(function(window){var BASE_URL="https://api.n.m28.io";var isSecure=window.location.protocol!="http:";function findServers(endpoint,cb){get(BASE_URL+"/endpoint/"+endpoint+"/findEach/",cb);}
...
findServers(endpoint,function(err,r){if(err)return cb(err);if(!r)return cb("Unknown error");if(!r.servers)return cb("Invalid response");var availableRegions=[];for(var region in r.servers){availableRegions.push(region);}
